...
def _build_formal_assessment(work: Path, scan_id: int) -> dict:
    """Build formal assessment artifacts from accepted structured outputs."""
...
def _build_formal_risk_map(work: Path) -> str:
    ...
    if not structure_json.is_file():
        structure_json = postprocess / "structure.json"
    return str(render_formal_risk_figure(
...
def _run_slam3r_pipeline(
    ...
    # 这里只读取 measurements/structure，不重跑或改写 SLAM3R 点云。
    ...
    except Exception:  # noqa: BLE001 - 评估失败不应丢失重建产物
