...
void main() {
  ...
  test('Scan.done/failed 判定', () {
    expect(Scan.fromJson({'id': 1, 'project_id': 2, 'status': 'done'}).done, true);
    expect(Scan.fromJson({'id': 1, 'project_id': 2, 'status': 'failed'}).failed, true);
    expect(Scan.fromJson({'id': 1, 'project_id': 2, 'status': 'sfm'}).done, false);
  });
  ...
  test('Report.fromJson 解析风险与评分', () {
    final r = Report.fromJson({'scan_id': 1, 'score': 62.5,
      ...
      'advice': ['建议扩门'], 'images': [], 'calibrated': 1});
    expect(r.score, 62.5);
