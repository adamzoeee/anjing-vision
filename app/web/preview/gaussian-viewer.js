...
async function main() {
    ...
    const requestedCameras = params.get("cameras");
    if (requestedCameras) {
        const cameraResponse = await fetch(new URL(requestedCameras, location.href), {
            mode: "cors",
            credentials: "omit",
            headers,
        });
        if (!cameraResponse.ok)
            throw new Error(cameraResponse.status + " Unable to load camera poses");
        const loadedCameras = await cameraResponse.json();
        if (!Array.isArray(loadedCameras) || loadedCameras.length === 0)
            throw new Error("Camera poses are empty");
    ...
    try {
        if (hashParams) throw new Error("token fragment is not a camera view");
        viewMatrix = JSON.parse(decodeURIComponent(location.hash.slice(1)));
        carousel = false;
    } catch (err) {}
    const requestedUrl = params.get("url");
    const url = new URL(
        requestedUrl || "train.splat",
        requestedUrl
            ? location.href
